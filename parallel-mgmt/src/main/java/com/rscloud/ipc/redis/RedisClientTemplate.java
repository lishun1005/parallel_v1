...
public class RedisClientTemplate {
	protected final Logger logger = LoggerFactory.getLogger(RedisClientTemplate.class);
	@Autowired
	...
	public Set<byte[]> keys2byte(String pattern) {
		...
		try {
			...
			result = j.keys(pattern.getBytes());
	...
	public Long append(byte[] key, byte[] value,int seconds) {
		Long result = null;
		ShardedJedis shardedJedis = getRedisClient();
		if (shardedJedis == null) {
			return result;
