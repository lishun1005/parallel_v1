...
public class ExportDir extends Configured implements Tool{
	private static Logger LOG = LoggerFactory
			.getLogger(ExportDir.class);
	...
	static class MapperCompress extends TableMapper<Text, BytesWritable> {
		...
		public void map(ImmutableBytesWritable key, Result values,
			...
			System.out.println("key is : " + Bytes.toString(key.copyBytes()));
			...
			if (0 <= Long.parseLong(size)) {
				...
				byte[] dfs = values.getValue(Bytes.toBytes("atts"),
						Bytes.toBytes("dfs"));			
				ZipEntry entry = new ZipEntry(zipKey);
				...
				if ( dfs[0] == '0') {
					...
					while ( (readLen = in.read(data, 0, 1024)) != -1){
						zos.write(data, 0, readLen);
					}
					in.close();
				}
			}else{
...
//		conf.setInt("mapreduce.map.memory.mb", 3072);
		...
		job.setJarByClass(ExportDir.class);
	...
	public static void main(String[] args) throws Exception {
		args = new String[]{"test_jonid_"+RandomUtils.nextInt(999),"/src//test","//src","1"};
		if (args.length < 4) {
			LOG.info("usage: <jobid> <source_path> <outputpath_gtdata> <zip_flag>");
			LOG.info("jobid");
			...
			int exitcode = ToolRunner.run(new ExportDir(), args);
