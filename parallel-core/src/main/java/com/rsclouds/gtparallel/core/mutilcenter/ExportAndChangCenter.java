...
public class ExportAndChangCenter {
  private static final Log LOG = LogFactory.getLog(ExportAndChangCenter.class);
  ...
  throws IOException {
    ...
    job.setJarByClass(ExportAndChangCenter.class);
    // Set optional scan parameters
    Scan s = getConfiguredScanForJob(conf, args);
    ResTableMapperFilter.initJob(tableName, s, ResTableMapperFilter.class, job);
 ...
 /* private static Scan getConfiguredScanForJob(Configuration conf, String[] args) throws IOException {
  ...
  private static Scan getConfiguredScanForJob(Configuration conf, String[] args) throws IOException {
	    ...
	    // Optional arguments.
	    // Set Scan Range
	    String startRow = args.length > 2?args[2] : null;
	    String stopRow = args.length > 3?args[3] : null;
	    if(startRow!=null)
	    ...
	    if (conf.get(TableInputFormat.SCAN_COLUMN_FAMILY) != null) {
	    	LOG.info("SCAN_COLUMN_FAMILY : " +  conf.get(TableInputFormat.SCAN_COLUMN_FAMILY));
	    	s.addFamily(Bytes.toBytes(conf.get(TableInputFormat.SCAN_COLUMN_FAMILY)));
	    }
	    // Set RowFilter or Prefix Filter if applicable.
  ...
  private static void usage(final String errorMsg) {
    ...
    System.err.println("Usage: Export <tablename> <outputdir> [<start_row> <stop_row> ]\n");
...
//	args = new String[]{"meta","nanlin_root/export-meta-test","/users/","/users/{","1420300860000"};
