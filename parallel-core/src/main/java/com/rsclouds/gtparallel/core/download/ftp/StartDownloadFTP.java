package com.rsclouds.gtparallel.core.download.ftp;
...
public class StartDownloadFTP {
	...
	public StartDownloadFTP(DownloadBeanFTP bean, String jobid,
			String downloadURL, String saveFilename, String path) {
		this.bean = bean;
		this.jobid = jobid;
	...
	public void startDownload() throws IOException {
		...
		// 建立FTP连接请求
		boolean isLogin = connect(bean.getIp(), bean.getPort(), bean.getUsername(),
				bean.getPassword());
		if(!isLogin){
			throw new IOException("FTP登录不成功，可能用户名密码错误");
		}
		...
		if (fileLength <= 0) {
			...
			throw new IOException("服务器不能返回文件大小");
		}
		...
		if (!stop) {
			...
			while (!stop) {
				try {
					...
					log.info(getDesc());
					// 返回下载进度
					DownloadUtils.pushProgress(jobid, getProgress(fileLength, downloaded),null);
					stop = true;
			...
			// 写入进度100
			log.info("开始返回100进度");
			DownloadUtils.pushProgress(jobid, "100",null);
			// 上传到gt-data
			log.info("开始上传文件到gt-data");
			Path localPath = new Path(CoreConfig.DOWNLOAD_TEMP_PATH,saveFilename);
			Path destPath = new Path(path,saveFilename);
			log.info("本地路径为:"+ localPath.toString());
			log.info("上传路径为:" + destPath.toString());

			Import importData = new Import();
			boolean flag = importData.ImportFromLocal(localPath.toString(), destPath.toString());
			if (flag) {
				log.info("上传gt-data成功");
				JobHbase.setJobState(jobid, CoreConfig.JOB_STATE.SUCCEEDED,System.currentTimeMillis());
				DownloadUtils.pushProgress(jobid, "100",CoreConfig.JOB_STATE.SUCCEEDED.name());
				log.info("开始删除临时文件");
				// 删除下载位置缓存文件
				...
				log.info("上传Hdfs失败");
				log.info("写入数据库失败");
				throw new IOException("上传Hdfs失败");
		   }	
		}
	}
	...
	private String getDesc() {
		return String
				.format("已下载/总大小=%s/%s(%s),速度:%s,耗时:%s,剩余大小:%d",
						...
						getFileFormat(downloadednow / ((System.currentTimeMillis() - beginTime) / 1000 + 1)),
