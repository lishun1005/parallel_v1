...
public class DeleteRedundancyData extends Configured implements Tool{
	public static Logger LOG = LoggerFactory.getLogger(DeleteRedundancyData.class);
...
//			jedis = new Jedis("192.168.2.6", GtDataConfig.REDIS_PORT, 30000);
			cleanFlag = conf.getBoolean("clean_flag", false);
		}
		...
		public void reduce(Text key, Iterable<Text>value, Context context)throws
			...
			for(Text val : value) {
				if(val.find("tar.gz") != -1) {
					notDeleteFlag = true;
				}else if(val.find(pathFilter) != -1) {//匹配需要删除的路径
					deleteFlag = true;
...
//								System.out.println(jedis.del(splits[0]));
			...
			if(deleteFlag && notDeleteFlag == false && testFlag == false) {
				if (deleteFile(resTable, fs, splits[0])) {
					pathText.set("hdfs delete sucessed");
					context.write(key, pathText);
					totalSize += fileSize;
				}
				else {
	...
	public int run(String[] args) throws Exception {
		...
		job.setJarByClass(DeleteRedundancyData.class);
...
//		args[3] = "1";
//		args[4] = "true";
		int status = ToolRunner.run(new DeleteRedundancyData(), args);
		System.exit(status);
	}
